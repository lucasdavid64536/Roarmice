...
class Client(ClientHandler):
        ...
        if this.validatingVersion:
            ...
            if C == 28 and CC == 1:
                ...
                if not ckey == this.server.CKEY:
                    this.server.sendOutput("WARNING: CKEY Errada, CKEY Certa: ("+ckey+")")
                    this.transport.loseConnection()

                if not validVersion == this.server.Version:
                    this.server.sendOutput("WARNING: Versao Da SWF Errada, Versao Certa: ("+validVersion+")")
                    this.transport.loseConnection()
    ...
    def loginPlayer(this, playerName, password, startRoom):	
        if this.server.connectedCounts[this.ipAddress] >= 3:
            this.sendPlayerBan(0, "Proibido farm em MiceMaster - Ban Permanente.", True)
            this.transport.loseConnection()
        playerName = "Turista" if playerName == "" else playerName
        if password == "":
        ...
        if len(playerName) < 3 or len(playerName) > 12:
            ...
            this.sendMessage("<br><N>«<J> Bem-Vindo Ao MiceMaster <N>»<br><N>Para ligar a rádio digite /play para desligar /stop<br><N>«<font color='#07C1AB'> Digite <N>/comandos<font color='#07C1AB'>, para ver a lista de comandos e ajudas</font><N> »<br><N>Versao 1.373 ==P<br><N>Source Criada Por: Andriel9 e Legion<br>")
    ...
    def playerWin(this, holeID, distance=-1):
        if distance != -1 and distance != 1000 and this.isSuspect and this.room.countStats:
            if distance >= 30:
                this.server.sendModMessage(7, "O usuário <V>"+this.Username+"<BL> foi kickado do MiceMaster por Atividade Suspeita.")
                this.sendPlayerBan(0, "Atividade suspeita", False)
                return
        ...
        if canGo:
            ...
            if place == 1:
                ...
                if this.room.getPlayerCountUnique() >= this.server.needToFirst and this.room.countStats and not this.isShaman and not this.canShamanRespawn and not this.isGuest:
                    ...
                    if not this.isGuest:
                        this.sendMessage ("<V>[•]<BL> Você chegou em primeiro na toca e ganhou <V>500</V> firsts e <V>200<BL> moedas.")
                    if this.room.isRacing or this.isGuest: 
                        this.room.sendAllBin(Identifiers.send.Message, ByteArray().writeUTF("<V>[•] <BL>Trocando Mapa em <V>25<BL> Segundos").toByteArray())
                ...
                if not this.isGuest:
                    this.sendMessage ("<V>[•]<BL> Você chegou em primeiro na toca e ganhou <V>300</V> firsts e <V>150<BL> moedas.")
                ...
                if not this.isGuest:
                    this.sendMessage ("<V>[•]<BL> Você chegou em primeiro na toca e ganhou <V>150</V> firsts e <V>50<BL> moedas.")       
    ...
    def sendAllModerationChat(this, type, message):
        playerName = this.Username if type == -1 else "" if type == 0 else "Mensagem Do Servidor" if type == 1 else this.Langue.upper() + "][" + ("Fundador][" if this.privLevel >= 11 else "Admin][" if this.privLevel >= 10 else "Coord][" if this.privLevel == 9 else "Smod][" if this.privLevel == 8 else "Mod][" if this.privLevel == 7 else "MapCrew][" if this.privLevel == 6 else "Ajudante][" if this.privLevel == 5 else "")
    ...
    def sendStaffLogin(this, isDisconnect):
        playerName = "" + ("Fundador" if this.privLevel >= 11 else "Admin" if this.privLevel >= 10 else "Coord" if this.privLevel == 9 else "SMod" if this.privLevel == 8 else "Mod" if this.privLevel == 7 else "MapCrew" if this.privLevel == 6 else "Ajudante" if this.privLevel == 5 else "")
        this.server.sendStaffChat(2, this.Langue, Identifiers.send.Staff_Chat, ByteArray().writeByte(2).writeUTF(playerName).writeUTF(this.Username + " <N>desconectou-se." if isDisconnect else this.Username + " <N>conectou-se.").writeInt(0).toByteArray())
        ...
        if rs:
            ...
            if diffDays <= 0:
                ...
                if this.TitleNumber >= 1000:
                    ...
                    this.sendClientMessage("O seu VIP se estogou. Você não poderá mais usar os sistemas e comandos exclusivos para VIP's.")
                    this.Cursor.execute("update Users set VipTime = 0 where Username = ?", [this.Username])
            else:
                this.sendClientMessage("Você ainda tem <V>"+str(diffDays)+"<BL> dias de privilégio VIP!")
...
class Server(protocol.ServerFactory):
    ...
    def __init__(this):
        ...
        this.blackList = this.parseJson("./Mice/JSON/BL.json")
    ...
    def setServerSetting(this, setting, value):
        ...
        with open("./Mice/Config.ini", "w") as configfile:
    ...
    def updateBlackList(this):
        ...
        with open("./Mice/JSON/BL.json", "w") as f:
            f.write(T)
    ...
    def sendPanelRestartMessage(this, seconds):
        if seconds == 120:
            this.sendOutput("[MiceMaster] O servidor vai reiniciar em 2 minutos.")
        elif seconds < 120 and seconds > 1:
            this.sendOutput("[MiceMaster] O servidor vai reiniciar em "+str(seconds)+" segundos.")
        else:
            this.sendOutput("[MiceMaster] O servidor vai reiniciar agora.")
    ...
    def sendEmail(this, code, DestinEmail, playerName):
        setHostName = "MiceMaster"
        setSslSmtpPort = 587
        setFrom = this.EmailAddress
        addTo = str(DestinEmail)
        setSubject = "MiceMaster"
        setMsg = "Olá "+playerName+", seu código de ativação é: "+str(code)
    ...
    def banPlayer(this, playerName, bantime, reason, modname, silent):
        ...
        if client != None:
            ...
            if not modname == "Server":
                ...
                this.sendModMessage(5, "<V>MiceMaster <BL>baniu o jogador <V>"+playerName+"<BL> por <V>1 <BL> hora. Motivo: <V>Vote Populaire<BL>.")
            ...
            if client.privLevel <= 2:
                if not myIP in client.voteBan:
                    ...
                    if len(client.voteBan) >= 10:
                        this.banPlayer(playerName, 1, "Vote Populaire", "MiceMaster", False)
    ...
    def sendModChat(this, senderClient, eventTokens, data, binary = None):
        if eventTokens == "\x1A\x04":
            print str(datetime.today())+" [MiceMaster] "+data[0]
    ...
    def mutePlayer(this, playerName, time, reason, modname):
        client = this.players.get(playerName)
        if client != None:
            this.sendModMessage(5, "<V>"+str(modname)+"<BL> deixou <V>"+playerName+"<BL> sem falar por <V>"+str(time)+"<BL> horas. Motivo: <V>"+str(reason)+"<BL>.")
    ...
    def getPlayersCountMode(this, mode, langue):
        modeName = "MiceMaster" if mode == 1 else "MiceMaster vanilla" if mode == 3 else "MiceMaster survivor" if mode == 8 else "MiceMaster racing" if mode == 9 else "MiceMaster music" if mode >= 10 else "MiceMaster bootcamp" if mode == 2 else "MiceMaster defilante" if mode >= 10 else "MiceMaster village" if mode == 16 else ""
        playerCount = 0
        for room in this.rooms.values():
    ...
    def checkMessage(this, client, message):
        ...
        while i < len(list):
            if re.search("[^a-zA-Z]*".join(list[i]), message.lower()):
                this.sendModMessage(7, "[<V>"+client.roomName+"<BL>] <font color='#1EC9B5'>"+client.Username+"</font> <BL>Digito um Message De a <V>BlackList<BL>: <BL>[ <font color='#D8D931'>"+str(message)+"</font><BL> ].")
                return True
            i += 1
    ...
    def setVip(this, playerName, days):
        ...
        if client != None:
            if client.privLevel == 1:
                ...
                client.privLevel = 2
                this.sendModMessage(7, "<V>"+playerName+"<BL> se tornou Vip do MiceMaster por <V>"+str(days)+"<BL> dias.")
        else:
    ...
    def checkSuspectBot(this, playerName, type):
        if not this.DEBUG:
            if this.banPlayer(playerName, 360, "Hack Suspect", "MiceMaster", False):
                this.sendModMessage(5, "<V>MiceMaster <BL>baniu <V>"+playerName+"<BL> por <V>360 <BL>horas. Motivo: <V>Hack Suspect ("+str(type)+")<BL>.")
...
class Room:
    def __init__(this, server, name):
        ...
        if this.roomName.startswith(chr(3) + "[Editando Mapa] "):
        ...
        elif this.roomName.startswith(chr(3) + "[Editando Totem] "):
            this.countStats = False
            this.specificMap = True
            this.currentMap = 444
    ...
    def checkVanillaXML(this):
        if os.path.exists("./Mice/Mapas/"+str(this.currentMap)+".xml"):
            FileXML = open("./Mice/Mapas/"+str(this.currentMap)+".xml")
            XML = FileXML.read()
            FileXML.close()

            this.mapCode = int(this.currentMap)
            this.mapName = "MiceMaster"
            this.mapXML = str(XML)
            this.mapYesVotes = 0
            this.mapNoVotes = 0
            this.mapPerma = 2
    ...
    def getMap801(this, prefix, map, code, name):
        try:
            with open("./Mice/Mapas/801.xml", "r") as f:
                XML = f.read()
                f.close()
...
if __name__ == "__main__":
    ...
    Config.read("./Mice/Config.ini")

    # Connection Database
    Database, Cursor = None, None
    Database = sqlite3.connect("./Mice/Data/Database.db", check_same_thread = False)
    ...
    # Connection Server
    os.system("title Source do Andriel9 e Legion v1.373")
    os.system('color fc')
    validPorts = []
    TFM = Server()
    for port in [443, 44440, 44444, 5555, 3724, 6112]:
        try:
            ...
            print "Portas Restantes: " + str(port)

    if not validPorts == []:
        print "Server RUNNING                  Portas Rodando: " + str(validPorts)
